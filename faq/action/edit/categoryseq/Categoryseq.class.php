 ...
 * @access      public
 */
class Faq_Action_Edit_Categoryseq extends Action
{
    // リクエストパラメータを受け取るため
    var $block_id = null;
    ...
    function execute()
    {
    	...
    	$cat = $this->db->selectExecute("faq_category",$params);
    	...
    	$drop_cat = $this->db->selectExecute("faq_category",$params);
    	...
    	$result = $this->db->seqExecute("faq_category", $params, $sequence_param);
    	if($result === false) {
    		return 'error';
    	}
    	...
    	$result = $this->db->seqExecute("faq_category", $params, $sequence_param, 1);
    	if($result === false) {
    		return 'error';
    	}
    	...
    	$result = $this->db->updateExecute("faq_category", $params, $where_params, true);
    	if($result === false) {
    		return 'error';
    	}
